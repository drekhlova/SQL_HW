CREATE DATABASE lesson2; -- IF NOR EXIST перед именем таблицы создаст её, если она не существует
...
-- DDL (определение данных): CREATE, ALTER (для изменения таблиц), DROP (для удаления), RENAME (для переименования), TRUNCATE (очистка всей таблицы)
...
-- DDL ALTER ALTER
-- Добавляет в таблицу столбец категория с типом данных INT
-- Добавляет в таблицу столбец цена с значением по дефолту 100
ALTER TABLE cinema
