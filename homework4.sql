...
-- 2 способ: используем подзапрос c оператором IN
SELECT shopname
FROM shops
WHERE id IN
(SELECT shops.id FROM cats WHERE shops.id = cats.shops_id AND cats.name = 'Murzik'); 
...
FROM shops
WHERE id NOT IN 
(SELECT shops_id FROM cats WHERE cats.name IN ('Murzik', 'Zuza'));
...
-- Добавила дату заказа для самопроверки

SELECT an_name AS 'Анализ', an_price 'Цена анализа, руб.', ord_datetime 'Дата заказа'
FROM analysis
